...
(function ($) {
  ...
  function SlickGrid(container, data, columns, options) {
    ...
    var $style;
    var $boundAncestors;
    var $headerParents, $headerParentsL, $headerParentsR;
    ...
    function init() {
      ...
      $headerParentsL = $("<div/>").appendTo($headerScrollerL);
      $headerParentsL.width(headersWidthL);
      $headerParentsR = $("<div/>").appendTo($headerScrollerR);
      ...
      $viewportTopL = $("<div class='slick-viewport slick-viewport-top slick-viewport-left' tabIndex='0' hidefocus='true' />").appendTo($paneTopL);
      $viewportTopR = $("<div class='slick-viewport slick-viewport-top slick-viewport-right' tabIndex='0' hidefocus='true' />").appendTo($paneTopR);
      $viewportBottomL = $("<div class='slick-viewport slick-viewport-bottom slick-viewport-left' tabIndex='0' hidefocus='true' />").appendTo($paneBottomL);
      $viewportBottomR = $("<div class='slick-viewport slick-viewport-bottom slick-viewport-right' tabIndex='0' hidefocus='true' />").appendTo($paneBottomR);
      ...
      $canvasTopL = $("<div class='grid-canvas grid-canvas-top grid-canvas-left' tabIndex='0' hidefocus='true' />").appendTo($viewportTopL);
      $canvasTopR = $("<div class='grid-canvas grid-canvas-top grid-canvas-right' tabIndex='0' hidefocus='true' />").appendTo($viewportTopR);
      $canvasBottomL = $("<div class='grid-canvas grid-canvas-bottom grid-canvas-left' tabIndex='0' hidefocus='true' />").appendTo($viewportBottomL);
      $canvasBottomR = $("<div class='grid-canvas grid-canvas-bottom grid-canvas-right' tabIndex='0' hidefocus='true' />").appendTo($viewportBottomR);
    ...
    // /TODO: old code below; new code to merge above
    ...
    function createColumnHeaders() {
      ...
      $headerL.empty();
      $headerR.empty();
      $headerParentsL.empty();
      $headerParentsR.empty();
      ...
      function createBaseColumnHeader(m, $headerTarget, $headerRowTarget, level, cell) {
        var header = createColumnHeader(m, $headerTarget, level, cell);
        var i, j, column;
      ...
      if (hasNestedColumns) {
        for (var i = 0; i < nestedColumns.length; i++) {
          ...
          var $headerParentTarget = (options.frozenColumn > -1) ? ((i <= options.frozenColumn) ? $headerParentsL : $headerParentsR) : $headerParentsL;

          var isParent = false;
          var layer = nestedColumns[i];
          if (i + 1 < nestedColumns.length) {
            $row = $("<div class='slick-header-columns slick-header-parents level" + i + "' style='left:-1000px' />").appendTo($headerParentTarget);
            isParent = true;
          }
          for (var j = 0; j < layer.length; j++) {
            ...
            if (isParent) {
              ...
              createBaseColumnHeader(column, $headerTarget, $headerRowTarget, i, j);
            }
          }
        }
      } else {
        for (var i = 0; i < columns.length; i++) {
          ...
          createBaseColumnHeader(m, $headerTarget, $headerRowTarget, 0, i);
        }
      }

      $headerL.addClass('level' + (hasNestedColumns ? nestedColumns.length - 1 : 0));
      $headerR.addClass('level' + (hasNestedColumns ? nestedColumns.length - 1 : 0));
    ...
    function setupColumnReorder() {
      ...
      var viewportLeft = $viewportScrollContainerX.offset().left;
    ...
    function scrollTo(y) {
      y = Math.max(y, 0);
      y = Math.min(y, virtualTotalHeight - $viewportScrollContainerY.height() + ((viewportHasHScroll || options.frozenColumn > -1) ? scrollbarDimensions.height : 0));
    ...
    function appendRowHtml(stringArrayL, stringArrayR, row, range, dataLength) {
      ...
      if (options.frozenColumn > -1) {
        ...
        appendMetadataAttributes(stringArrayR, metadata);
        stringArrayR.push(">");
      }
      ...
      for (var i = 0, ii = columns.length; i < ii; i += colspan) {
        ...
        if (columnPosLeft[i + colspan] > range.leftPx) {
          ...
          if (options.frozenColumn > -1 && i > options.frozenColumn) {
            appendCellHtml(stringArrayR, row, i, colspan, metadata, columnData, d);
          } else {
            appendCellHtml(stringArrayL, row, i, colspan, metadata, columnData, d);
          }
      ...
      if (metadata && metadata.appendHtml) {
        stringArrayL.push(metadata.appendHtml);

        if (options.frozenColumn > -1) {
          stringArrayR.push(metadata.appendHtml);
        }
      }
    ...
    function cleanupRows(rangeToKeep) {
      for (var i in rowsCache) {
        ...
        i = i | 0;
        if (i !== activeRow && removeFrozenRow) {
    ...
    function resizeCanvas() {
      ...
      // TODO
      numVisibleRows = getRowWithFractionFromPosition(viewportTopH).position;
    ...
    function updateRowCount() {
      ...
      viewportHasVScroll = !options.autoHeight && (getRowBottom(numberOfRows - 1) > tempViewportH);
      ...
      if (scrollableHeight !== oldH) {
        if (hasFrozenRows && !options.frozenBottom) {
          $canvasBottomL.css("height", scrollableHeight);

          if (options.frozenColumn > -1) {
            $canvasBottomR.css("height", scrollableHeight);
          }
        } else {
          $canvasTopL.css("height", scrollableHeight);
          $canvasTopR.css("height", scrollableHeight);
        }
      ...
      if (virtualTotalHeight === 0 || scrollTop === 0) {
        ...
        // scroll to bottom
        scrollTo(virtualTotalHeight - tempViewportH);
      }
    ...
    function renderRows(range) {
      ...
      for (i = 0, ii = rows.length; i < ii; i++) {
        ...
        if (isBrowser.safari605) {
          ...
          rowsCache[rows[i]].rowNode[0].offsetWidth;
        }
      }
    ...
    */
    ...
    function getCellNodeBox(row, cell) {
      ...
      // TODO: line above vs. block below
/*
      var x1 = 0;
...
*/
    ...
    function setActiveCellInternal(newCell, opt_editMode) {
      ...
      if (activeCellNode != null) {
   	    // TODO: next line is ignored in merged code!
    ...
    function scrollRowIntoView(row, doPaging, doCentering) {
      ...
      var height = viewportScrollH - (viewportHasHScroll ? scrollbarDimensions.height : 0);
      ...
      // need to page down?
      if (getRowBottom(row) > scrollTop + viewportScrollH + pageOffset - (viewportHasHScroll ? scrollbarDimensions.height : 0)) {
