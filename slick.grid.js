...
(function ($) {
  ...
  function SlickGrid(container, data, columns, options) {
    ...
    var defaults = {
      ...
      navigateCancelsInvalidEdits: true,
    ...
    function init() {
      ...
      $paneHeaderL = $("<div class='slick-pane slick-pane-header slick-pane-left' tabIndex='0' role='row' />").appendTo($container);
      $paneHeaderR = $("<div class='slick-pane slick-pane-header slick-pane-right' tabIndex='0' role='row' />").appendTo($container);
      $paneTopL = $("<div class='slick-pane slick-pane-top slick-pane-left' tabIndex='0' role='row' />").appendTo($container);
      $paneTopR = $("<div class='slick-pane slick-pane-top slick-pane-right' tabIndex='0' role='row' />").appendTo($container);
      $paneBottomL = $("<div class='slick-pane slick-pane-bottom slick-pane-left' tabIndex='0' role='row' />").appendTo($container);
      $paneBottomR = $("<div class='slick-pane slick-pane-bottom slick-pane-right' tabIndex='0' role='row' />").appendTo($container);
    ...
    function setupColumnReorder() {
	  if (!(jQuery.isEmptyObject($.data( $headers, $headers.sortable.prototype.widgetFullName ) ))){
        ...
        var sortable_headers = $headers.filter(":ui-sortable");
		sortable_headers.sortable();
        sortable_headers.sortable("destroy");
	  }
...
//                return false;
    ...
    function getDataItem(i) {
      ...
      if (data.getItem) {
        $.when(data.getItem(i)).done(function (itm) {
          itemDeferred.resolve(itm);
        });
      } else {
        itemDeferred.resolve(data[i]);
      }
    ...
    function getRowTop(row) {
      var o = getFrozenRowOffset(row);
      return options.rowHeight * row - o;
    }
    ...
    function getEditor(row, cell, item) {
      var promise = new $.Deferred();

      var column = columns[cell];

      if (data.getEditor) {
        $.when(data.getEditor(row, column, item)).done(function (editor) {
        ...
        if (rowMetadata && rowMetadata.editable === false) {
    ...
    function appendRowHtml(stringArrayL, stringArrayR, row, range, dataLength) {
      ...
      var rowHtml = "<div class='ui-widget-content role='row' " + rowCss + "' style='top:"
    ...
    function appendCellHtml(stringArray, row, cell, colspan, item) {
        ...
        function getAddtlCss(row, col){
          var s = "";
          for (var key in cellCssClasses) {
            if (cellCssClasses[key][row] && cellCssClasses[key][row][m.id]) {
              s += (" " + cellCssClasses[key][row][m.id]);
            }
          }
          return s;
        }

        cellCss += getAddtlCss(row,m.id);

        stringArray.push("<div class='" + cellCss + "' aria-describedby='" + uid + m.id + "' tabindex='-1' role='gridcell'></div>");
        // if there is a corresponding row (if not, this is the Add New row or this data hasn't been loaded yet)
        rowsCache[row].cellRenderQueue.push(cell);
        rowsCache[row].cellColSpans[cell] = colspan;

        // insert the data at a later time, whenever it is actually ready.
        $.when(item).done(function (d) {
          if (d) {
            _.defer(function(){
              var value = getDataItemValueForColumn(d, m);
              var o = getCellNode(row,cell);
    ...
    function updateRowCount() {
      ...
      if (hasFrozenRows && options.frozenBottom) {
        numberOfRows = dataLength - options.frozenRow;
      } else {
      ...
      if (cacheEntry && cacheEntry.rowNode) {
        if (cacheEntry.cellRenderQueue.length) {
          ...
          while (cacheEntry.cellRenderQueue.length) {
            ...
            if (options.frozenColumn > -1 && $lastNode.length == 0) {
    ...
    function renderRows(range) {
      ...
      for (var i = range.top, ii = range.bottom; i <= ii; i++) {
        if (rowsCache[i] || ( hasFrozenRows && options.frozenBottom && i == dataLength )) {
          continue;
        }
        renderedRows++;
        rows.push(i);
        ...
        $.when(getDataItem(row)).done(function (d) {
          for (var columnIdx in cacheEntry.cellNodesByColumnIdx) {
            ...
            if (m.asyncPostRender && !postProcessedRows[row][columnIdx]) {
              ...
              if (node) {
                m.asyncPostRender(node, row, d, m);
              }
    ...
    function handleClick(e) {
      ...
      if ((activeCell != cell.cell || activeRow != cell.row) && canCellBeActive(cell.row, cell.cell)) {
        $.when(getEditorLock().commitCurrentEdit()).done(function (commitResult) {
          if (options.navigateCancelsInvalidEdits && !commitResult) {
            cancelCurrentEdit()
          } else if (!options.navigateCancelsInvalidEdits && !commitResult) {
            return;
          }
          if (hasFrozenRows) {
    ...
    function getRowFromNode(rowNode) {
      for (var row in rowsCache) {
        var nodes = rowsCache[row].rowNode;
        for (var i = 0; i < nodes.length; i++) {
          if (rowsCache[row].rowNode[i] === rowNode[0]) {
            return row | 0;
          }
        }
      }
    ...
    function isCellPotentiallyEditable(row, cell) {
      ...
      var isEditable = new $.Deferred();
      $.when(getDataItem(row)).done(function (d) {
      if (row < dataLength && !d) {
          isEditable.resolve(false);
        } else if (columns[cell].cannotTriggerInsert && row >= dataLength) {
          isEditable.resolve(false);
    ...
    function makeActiveCellEditable(editor) {
      ...
      $.when(
          ...
          .done(function (item, isEditable) {
            if (isEditable) {
              $.when(editor || getEditor(activeRow, activeCell, item)).done(function (editor) {
    ...
    function navigate(dir) {
      var didNavigate = $.Deferred();
      if (!activeCellNode || !options.enableCellNavigation) {
        didNavigate.resolve(false);
      }
      ...
      $.when(getEditorLock().commitCurrentEdit()).done(function (commitResult) {
        if (options.navigateCancelsInvalidEdits && !commitResult) {
          // commit failed, and we want to navigate away -- so we should
          // cancel the edit so moving can continue as desired.
          cancelCurrentEdit();
        } else if (!options.navigateCancelsInvalidEdits && !commitResult) {
          didNavigate.resolve(false);
          return; // return only returns out of the $.when block
        }
        ...
        if (pos) {
          ...
          scrollCellIntoView(pos.row, pos.cell, ( !options.frozenBottom && pos.row >= actualFrozenRow ) || ( options.frozenBottom && pos.row < actualFrozenRow ) ? !isAddNewRow : false);
    ...
    function gotoCell(row, cell, forceEdit) {
      ...
      if (!initialized || !canCellBeActive(row, cell)) {
        done.resolve(false);
        return done.promise();
      }

      $.when(getEditorLock().commitCurrentEdit()).done(function (passedCommit) {
        if (passedCommit) {
          scrollCellIntoView(row, cell, false);
              ...
              if (validationResults.valid) {
                $.when(currentEditor.serializeValue()).done(function (serializedValue) {
                  if (activeRow < getDataLength()) {
                    ...
                    var newItem = {};
                    currentEditor.applyValue(newItem, serializedValue);
                    makeActiveCellNormal();
