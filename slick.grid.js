...
(function ($) {
   ...
   * @param {Array}             columnsInput      An array of column definitions.
  ...
  function SlickGrid(container, data, columnsInput, options) {
    ...
    function updateCanvasWidth(forceColumnWidthsUpdate) {
      ...
      if (canvasWidth != oldCanvasWidth) {
        ...
        $headerParents.width(getHeadersWidth());
      ...
      if (hasNestedColumns) {
        for (var i = 0; i < nestedColumns.length; i++) {
          ...
          if (i + 1 < nestedColumns.length) {
            $row = $("<div class='slick-header-columns slick-header-parents' style='left:-1000px' />").appendTo($headerParents);
            isParent = true;
          }
          for (var j = 0; j < layer.length; j++) {
            var column = layer[j];
            if (isParent) {
              createColumnHeader(column, $row);
            }
            else {
              createBaseColumnHeader(column);
            }
          }
        }
      }
    ...
    function setupColumnReorder() {
      ...
      $headers.sortable({
        ...
        stop: function (e) {
          ...
          trigger(self.onColumnsReordered, { srcElementId: e.srcElement.id, columnsInput: columnsInput });
          e.stopPropagation();
          setupColumnResize();
        }
      });
    ...
    function getDataItemValueForColumn(item, columnDef) {
      ...
      if (columnDef.field && columnDef.field.indexOf('.') > 0) {
    ...
    function appendRowHtml(stringArray, row, range, dataLength) {
      ...
      var titleText = '';
      if (metadata && metadata.titleText) {
        titleText = metadata.titleText;
      }

      stringArray.push("<div class='ui-widget-content " + rowCss + "' style='top:" + getRowTop(row) + "px' title='" + titleText + "' role='row'>");
    ...
    function renderRowCache() {
      rowPositionCache = []; /*Need to reset so that when rows are filtered we can get correct context for surround visible rows*/
      for (var row = 0; row <= numberOfRows; row++) {
        ...
        if (metadata && metadata.cssClasses.indexOf('slick-group-visible') > -1) {
          currRowHeight += options.groupRowHeight - options.rowHeight;
        }

        if (metadata && metadata.cssClasses.indexOf('slick-group-hidden') > -1) {
          currRowHeight = 0;
        }

        if (metadata && metadata.cssClasses.indexOf('slick-first-hidden-row') > -1) { /*First contiguous hidden row.*/
          currRowHeight += options.hiddenRowHeight - options.rowHeight;
        }

        if (metadata && metadata.cssClasses.indexOf('slick-trailing-hidden-row') > -1) { /*At least one contiguous prior row hidden*/
          currRowHeight = 0;
        }
