...
(function ($) {
    ...
    function SlickGrid(container, data, columns, options) {
        ...
        var rowSizeManagerDefault = {
            getRowTop: getRowTop,
            getRowHeight: getRowHeight,
            getRowFromPosition: getRowFromPosition,
            init: rowSizeManagerInit,
        ...
        function init() {
            ...
            if (typeof(rowSizeManager) == 'undefined'){
                rowSizeManager = rowSizeManagerDefault;
            }
        ...
        function rowSizeManagerInit(){

        }

        function setRowSizeManager(rowSizeMgr){
            if (!rowSizeMgr)
            {
                rowSizeManager = rowSizeManagerDefault;
            } else {
                rowSizeManager = rowSizeMgr;
            }
        }

        function isAutoHeightEnabled(){
            return false;
        }
        ...
        function createCssRules() {
            ...
            var rules = [
                ...
                /*
                 cb: disable for variable row height
                 ,
        ...
        var getRowTop = function(row) {
            return options.rowHeight * row - offset;
        }

        var getRowFromPosition = function(y) {
        ...
        function appendRowHtml(stringArray, row, range, dataLength) {
            ...
            if( !rowSizeManager.isAutoHeightEnabled){
            ...
            for (var i = 0, ii = columns.length; i < ii; i++) {
                ...
                if (columnPosRight[Math.min(ii - 1, i + colspan - 1)] > range.leftPx) {
                    ...
                    if(rowSizeManager.isAutoHeightEnabled) {
                        ...
                        if (Number(cellHeight[1]) > tallest) {
                            tallest = Number(cellHeight[1]);
                        }
                        for (var j = 0; j < cellStringArray.length; j++) {
                            cellStringsToAppend.push(cellStringArray[j]);
                        }
                    } else {
            ...
            if(rowSizeManager.isAutoHeightEnabled){
                rowSizeManager.setRowHeight(row, tallest);
                stringArray.push("<div class='ui-widget-content " + rowCss + "' style='top:" + (rowSizeManager.getRowTop(row) - offset) +  "px; min-height: " + tallest + "px'>");
        ...
        function appendCellHtml(stringArray, row, cell, colspan, item) {
            if(rowSizeManager.isAutoHeightEnabled ) {
                ...
                if (cellCache["row"] == undefined) {
                    ...
                    // moved on to a new row.  store the row height
                    if (row > 0) {

                        if ( rowSizeManager.getRowHeight(row - 1) != Number(cellCache.height)) {
                            rowSizeManager.setRowHeight(row - 1, Number(cellCache.height));
                        }
                    }
                    var h = cellCache["row"];
                    cellCache = {
            ...
            if(rowSizeManager.isAutoHeightEnabled ){
                ...
                if (dataItem["__group"] || dataItem["group"] || dataItem["groups"]) {
                    var sum = 0;
                    for(var i = cell; i < (cell+colspan); i++){
                        sum += Number($($(".slick-header-column")[i]).width());
                    }
                    sum += Number($($(".slick-header-column")[0]).width())
                    colWidth = String(sum) + "px"
                }
                ...
                // group header message
                    ...
                    cellCache.height = 0;
                    var storedCellCache = cellCache.height;
                    colWidth = String((getCanvasWidth() + scrollbarDimensions.width)) + "px";
                    $("#width_tester").css({"width": ''})
                    $("#width_tester").css({"width": colWidth})
                ...
                stringArray.push("<span class='" + cellCss + "' style='white-space:normal;word-wrap:break-word;height:" + cellCache.height + "px;colspan: "+ colspan +"'>");
            }
            else{
        ...
        function getVisibleRange(viewportTop, viewportLeft) {
            ...
            if(rowSizeManager.isAutoHeightEnabled){
                ...
                for (var i = 0; i < len; i++ ){
                    height += rowSizeManager.getRowHeight(i);

                    if(!topSet&&height>=(viewportTop+offset)){
                        topSet = true;top = i;
                    }
                    // this isn't being set
                    if((height+rowSizeManager.getRowHeight(i))>(viewportTop+offset+viewportH)){
                        bottom = i+1;break;
                    }
        ...
        function getRenderedRange(viewportTop, viewportLeft) {
            ...
            if (typeof(rowSizeManager.averageRowHeight) == 'function') {
                buffer = Math.round(viewportH / rowSizeManager.averageRowHeight());
            }
            /*
             else {
        ...
        function updateRowPositions() {
            for (var row in rowsCache) {
                rowsCache[row].rowNode.style.top = rowSizeManager.getRowTop(row) + "px";
            }
        }
        ...
        function render() {
            ...
            if(rowSizeManager.isAutoHeightEnabled){
                $canvas.css('height', rowSizeManager.getTotalHeight());
            }
        }
        ...
        function getCellNodeBox(row, cell) {
            ...
            var y1;
            if (rowSizeManager.isAutoHeightEnabled) {
                y1 = rowSizeManager.getRowTop(row) - offset;
            } else {
        ...
        function scrollRowIntoView(row, doPaging) {
            ...
            var rowAtBottom = 0;
            if (rowSizeManager.isAutoHeightEnabled){
                rowAtTop = rowSizeManager.getRowHeight(row)
                rowAtBottom = rowSizeManager.getRowHeight(row+1) - viewportH + (viewportHasHScroll ? scrollbarDimensions.height : 0);
                // need to page down?
                if (rowSizeManager.getRowHeight(row+1) > scrollTop + viewportH + offset) {
                    scrollTo(doPaging ? rowAtTop : rowAtBottom);
                    render();
                }
                // or page up?
                else if (rowSizeManager.getRowHeight(row) < scrollTop + offset) {
        ...
        function scrollRowToTop(row) {
            if(rowSizeManager.isAutoHeightEnabled){
                rowSizeManager.getRowTop(row);
                scrollTo(rowSizeManager.getRowTop(row));
            }
            else{
        ...
        function getRowSizeManager(){
            return rowSizeManager;
        }
        ...
        $.extend(this, {
            ...
            "setRowSizeManager": setRowSizeManager,
            "getRowSizeManager": getRowSizeManager
        });
